  ...
  try {
    ...
    if (startDate < currentDate) {
      return res
        .status(400)
        .send(
          '<script>alert("Cannot create a coupon with a start date in the past."); window.location="/couponManagement";</script>'
        );
    }
  ...
  try {
    ...
    if (cart.totalprice < coupon.minimumPurchase) {
      return res
        ...
        .json({
          error:
            "Total price does not meet the minimum purchase requirement for this coupon.",
        });
    }
