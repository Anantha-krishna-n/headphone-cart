...
exports.generateSalesReportPDF = async (req, res) => {
  ...
  try {
    ...
    orders.forEach((order) => {
      ...
      let x = 10; // Starting x position
      doc.text(rowIndex.toString(), x, y, {
        width: columnWidths[0],
        align: "center",
      });
      x += columnWidths[0];

      doc.text(order.user.name, x, y, {
      ...
      doc.text(order.paymentMethod, x, y, {
        width: columnWidths[5],
        align: "center",
      });
      x += columnWidths[5];

      doc.text(order.total.toString(), x, y, {
      ...
      doc.text(order.createdAt.toDateString(), x, y, {
